...
\end{lstlisting}
	\item \textbf{Abstract factory}: qui l'idea è di fornire un interfaccia per la realizzazione di famiglie di oggetti correlati o dipendenti fra di loro, senza dover specificare le loro classi concrete.

		Risulta un paradigma utile in contesti in cui le classi concrete da utilizzare cambiano in blocco, e si vuole quindi delegare la creazione non di un singolo oggetto, ma di una loro famiglia, a un singolo \textit{factory method}.
...
// istanze concrete PDF

class ParagraphPDF implements Paragraph {
	void show() {
		// ...
	}
}

class TablePDF implements Table {
	void show() {
		// ...
	}
}

// istanze concrete HTML
...
Per creare un documento, vorremo quindi usare più factory relative a documenti PDF e HTML, che ineritano dalla stessa \textit{abstract factory}:
\begin{lstlisting}[language=java, style=codestyle]	
...
// costruttore concreto HTML 
class DocumentFactoryHTML implements DocumentFactory {
	Paragraph createParagraph() {
...
class Document {
	...
	public Document(DocumentFactory factory) {
...
DocumentFactory factory = new DocumentFactoryPDF();
...
\end{lstlisting}
\item \textbf{Prototype}: qui vogliamo specificare i tipi di oggetti che vogliamo creare non come classi, ma sulla base di determinati oggetti predefiniti detti \textit{prototipi}. In Java, questo è possibile attraverso l'interfaccia \textit{Cloneable}, un interfaccia \textit{marker} (lo è ad esempio anche \textit{Serializable}), che segnala che un oggetto è clonabile attraverso il metodo \lstinline|clone()| della classe \textit{Object}. Notiamo che \lstinline|clone()| fa la \textit{shallow copy}.
...
public class Shape2D implements Cloneable {
	...
	public Shape2D(int size, String color) {
		this.size = size;
		this.color = color;
	}

	public void draw() {
		System.out.println("Drawing a 2D shape with size "
			+ size + " and color " + color);
	}

	@Override
	public Shape2D clone() throws CloneNotSupportedException {
		return (Shape2D) super.clone(); // shallow copy 
	}

	public static void main(String[] args) {
		Shape2D original = new Shape2D(10, "red");
		try {
			Shape2D copy = original.clone();
			original.draw();
			copy.draw();
		} catch (CloneNotSupportedException e) {
			System.err.println("Error during cloning");
		}
	}
}
\end{lstlisting}
